...
<div class="relative">
  ...
  <div class="relative flex flex-col w-full min-h-screen mx-auto lg:flex-row ">
    ...
    <div class="flex flex-col items-center justify-center w-full h-screen lg:w-1/2 bg-redjuli cursor-mano" @click="show = !show">
      ...
      <div v-if="$i18n.locale === 'fr'" class="relative w-48 h-48 mt-auto overflow-hidden bg-center bg-cover bg-jour-2">
      </div>
      <div v-else class="relative w-48 h-48 mt-auto overflow-hidden bg-center bg-cover bg-cuadro-2">
      </div>
      ...
      <div class="flex flex-col items-center justify-center w-full px-4 py-4 mx-auto mt-4 text-sm leading-normal text-center text-white lg:w-1/2">
        <p class="pb-6">{{ $t('diadosfrase1')}}
        </p>
        <p class="uppercase">{{ $t('diadosfrase2')}}.</p>
      </div>
    </div>
    ...
    <div key="right1" v-if="show" @click="show = !show" class="relative flex flex-col w-full h-screen p-4 transition duration-200 ease-in-out bg-white group lg:w-1/2 cursor-mano">
      ...
      <div class="flex items-center justify-center w-full h-48">
        ...
        <div class="text-sm uppercase text-bluejuli">{{ $t('diadospalabra1')}} </div>
      </div>
      <div class="flex justify-around">
        <div class="flex items-center justify-center w-full h-48">
          <div class="px-1 py-1 mr-2 border border-bluejuli bg-bluejuli">
          </div>
          <div class="text-sm uppercase text-bluejuli">{{ $t('diadospalabra2')}} </div>
        </div>
        <div class="flex items-center justify-center w-full h-48">
          <div class="px-1 py-1 mr-2 border border-bluejuli bg-bluejuli">
          </div>
      ...
      <div class="flex items-center justify-center w-full h-48 space-x-16">
        <div class="flex items-center justify-center w-full">
          ...
          <div class="text-sm uppercase text-bluejuli">{{ $t('diadospalabra3')}} </div>
        </div>
...
export default {
  head: {
    title: 'Persona Diseño | Porfolio - Dia 2',
    meta: [
      {
        hid: 'description',
        name: 'description',
